      ...
      <div className="border-b border-stone-200 bg-white/70 backdrop-blur-sm">
        <div className="max-w-2xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-5">
            {plan === 'business' && (
              <Link href="/dashboard/all" className="text-sm text-stone-500 hover:text-[#195C4C] transition-colors">Combined view</Link>
            )}
            <Link href="/pricing" className="text-sm text-stone-500 hover:text-[#195C4C] transition-colors">Pricing</Link>
            <button onClick={handleLogout} className="text-sm text-stone-500 hover:text-[#195C4C] transition-colors">Log out</button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto p-6">
        ...
        {limitReached ? (
          <div className="bg-white border border-stone-200 rounded-2xl p-6 mb-6 text-center">
            <div className="w-10 h-10 bg-[#195C4C]/10 rounded-full flex items-center justify-center mx-auto mb-3">
              <svg className="w-5 h-5 text-[#195C4C]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <form onSubmit={handleAddSite} className="bg-white border border-stone-200 rounded-2xl p-5 mb-6 space-y-3">
            ...
            <input placeholder="Site name (e.g. My Blog)" value={name}
              ...
              className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#195C4C]/25 focus:border-[#195C4C]" required />
            <input placeholder="Domain (e.g. myblog.com)" value={domain}
              onChange={(e) => setDomain(e.target.value)}
              className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#195C4C]/25 focus:border-[#195C4C]" required />
            <button type="submit" disabled={loading} className="bg-[#195C4C] hover:bg-[#195C4C] text-white rounded-lg px-4 py-2 text-sm font-medium transition-colors disabled:opacity-50">
        ...
        <div className="space-y-3">
          {sites.map((site) => (
            <div key={site.id} className="bg-white border border-stone-200 rounded-2xl p-5">
              <Link href={`/dashboard/${site.id}`} className="flex items-center gap-2 font-medium text-stone-900 hover:text-[#195C4C] transition-colors">
