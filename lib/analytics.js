...
// The plan allowance is counted per calendar month, in UTC so every viewer and the
// server agree on where a month starts.
...
// The current buckets shifted back one period, so the previous period's counts line up
// with them (the dashed line). Null when the range has no previous period (all time).
export function previousBuckets(window) {
  if (!window.previous) return null
  return window.buckets.map((bucket) => (window.previous.shiftMonths
...
// Before 2026-09-26 the script also sent referrers from the site's own pages, so older
// rows can name the site itself. With the site's domain given, those read as Direct.
export function referrerName(referrer, siteDomain = null) {
  if (!referrer) return 'Direct / None'
  try {
    const host = new URL(referrer).hostname.replace(/^www\./, '')
    if (siteDomain && host === cleanDomain(siteDomain).split('/')[0].toLowerCase().replace(/^www\./, '')) return 'Direct / None'
...
// Share passwords are hashed in the browser and only the hash is stored, so a
// stolen sites table never hands over the password itself. Salted and stretched
// (PBKDF2), because a plain SHA-256 of a short password is a rainbow-table lookup:
// the salt makes every row its own problem and the iterations make each guess slow.
// Format: pbkdf2$<iterations>$<salt hex>$<hash hex>, which /api/share-auth parses.
// Older unsalted hex hashes still work there and are upgraded on the next correct
// password, so nobody has to set their password again.
