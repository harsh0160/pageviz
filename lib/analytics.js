...
export function downloadPageviewsCsv(site, pageviews, range) {
  ...
  const rows = pageviews.map((pv) => [pv.created_at, pv.page_url, pv.referrer || 'Direct', pv.device_type || 'Unknown'])
  ...
  link.href = url
  link.download = `${site.name.replace(/\s+/g, '-').toLowerCase()}-pageviews-${range === 'all' ? 'all' : `${range}d`}.csv`
  link.click()
